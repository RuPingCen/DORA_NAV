    ...
    const CostmapBuilder& costmap)
{
    ...
    for (int j = 0; j < config_.w_samples; ++j) {
        ...
        // 预测轨迹
        auto trajectory = predictTrajectory(current_state, v, w);
        ...
        if (score > best_score) {
            ...
            best_score_goal = calculateGoalScore(trajectory, local_path_x, local_path_y);
        }
    }
    ...
    if (std::isinf(best_score) && best_score < 0) {
        ...
        // std::cout << "      - Goal score:     " << best_score_goal
        //           << " (weight=" << config_.weight_goal << ")" << std::endl;
    }
    ...
    const RobotState& state, double v, double w)
{
    ...
    for (int i = 0; i <= steps; ++i) {
        ...
        if (std::abs(w) < 1e-6) {
            ...
            // 圆弧运动
            double radius = v / w;
            x += radius * (std::sin(yaw + w * config_.dt) - std::sin(yaw));
            y += radius * (std::cos(yaw) - std::cos(yaw + w * config_.dt));
            yaw += w * config_.dt;
        }
    }
    ...
    const CostmapBuilder& costmap)
{
    ...
    // 2. 终点得分
    double score_goal = calculateGoalScore(trajectory, path_x, path_y);

    // 加权总得分
    double total_score = config_.weight_obstacle * score_obstacle +
                        config_.weight_goal * score_goal;
...
// =====================================================================
//  3.2 终点得分（轨迹终点朝向与目标方向的夹角越小得分越高）
// =====================================================================
double DWAPlannerComponent::calculateGoalScore(
    const std::vector<std::pair<float, float>>& trajectory,
    const std::vector<double>& path_x,
    const std::vector<double>& path_y)
{
    ...
    // 计算从轨迹终点指向路径终点的向量
    double dx = goal_x - end_x;
    double dy = goal_y - end_y;
