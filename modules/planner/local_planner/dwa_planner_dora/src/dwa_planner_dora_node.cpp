...
private:
    ...
    void handleTimer() {
        ...
        if (isGoalReached()) {
            ...
            // 每10帧打印一次状态
            if (frame_count % 10 == 0) {
                std::cout << "[DWAPlanner] Goal reached, staying at target" << std::endl;
            }
            return;
        }
    ...
    bool isGoalReached() const {
        ...
        // 获取路径终点
        double goal_x = path_x_.back();
        double goal_y = path_y_.back();

        // 计算到终点的距离
        double dx = goal_x - current_state_.pose.x;
        double dy = goal_y - current_state_.pose.y;
