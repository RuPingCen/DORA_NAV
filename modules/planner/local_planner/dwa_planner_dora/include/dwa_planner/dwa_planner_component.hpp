...
struct DWAConfig {
    ...
    double max_w = 3.0;      // 最大角速度 [rad/s]（增大以提高机动性）

    // 加速度限制
    double max_accel_w = 6.0;   // 最大角加速度 [rad/s²]（扩大动态窗口到±0.5 rad/s）

    // 采样参数
    int w_samples = 20;          // 角速度采样数

    // 预测参数
    double predict_time = 3.2;   // 轨迹预测时间 [s]（减小以提高响应速度）
    double dt = 0.1;             // 预测时间步长 [s]

    // 评分函数权重（得分越高越好）
    double weight_obstacle = 3.0;  // 避障得分权重（提高优先级）
    double weight_goal = 0.6;      // 指向终点得分权重

    // 速度平滑参数
    double velocity_smooth_alpha = 0.9;  // 平滑系数 [0-1]，提高响应速度
};
