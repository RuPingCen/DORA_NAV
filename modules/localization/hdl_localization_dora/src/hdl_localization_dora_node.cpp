...
private:
    ...
    void handlePointCloudInput(const char* data, size_t len) {
        ...
        // Enqueue for async NDT processing
        {
            ...
            if (cloud_queue_.size() >= kMaxQueueSize) {
                cloud_queue_.pop();
                std::cerr << "[WARN] NDT queue full, dropping oldest frame" << std::endl;
            }
    ...
    void processPointCloud(const pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud, double timestamp) {
        ...
        // 打印点云到达间隔
        if (last_pointcloud_time_ > 0.0) {
            double cloud_interval = timestamp - last_pointcloud_time_;
            // std::cout << "[HDL] cloud_interval=" << cloud_interval * 1000.0 << "ms" << std::endl;
        }
        ...
        // std::cout << "[HDL] downsample=" << Ms(t1 - t0).count()
        //           << "ms  predict=" << Ms(t2 - t1).count()
        //           << "ms  NDT=" << Ms(t3 - t2).count()
        //           << "ms  publish=" << Ms(t4 - t3).count()
        //           << "ms  total=" << Ms(t4 - t0).count()
        //           << "ms  pts_in=" << cloud->size()
        //           << "  pts_filtered=" << filtered->size()
        //           << std::endl;
    ...
    void publishPose(const Eigen::Matrix4f& pose, double timestamp) {
        nlohmann::json json_data = JsonSerializer::poseToJson(pose, timestamp, pose_seq_++);
...
private:
    ...
    bool stop_worker_;
    static constexpr size_t kMaxQueueSize = 2;
